...
This vignette shows the use of scToppR within a differential expression workflow. Using the IFNB (Kang 2018) dataset included in the SeuratData package, one can find differentially expressed genes between the "CTRL" and "STIM" groups using Seurat's FindMarkers function. The raw results from this analysis are included as a dataset in scToppR, which can be accessed as such:
